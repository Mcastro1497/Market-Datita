...
export function ExcelUploader({ onUploadComplete }: ExcelUploaderProps) {
  ...
  const processExcelData = async (data: ExcelRow[]) => {
    console.log("[v0] Procesando", data.length, "filas del Excel")
    ...
    const { data: existingTickers } = await supabase.from("ons_flows").select("ticker").in("ticker", tickers)
    ...
    async (acceptedFiles: File[]) => {
      ...
      try {
        ...
        if (processedData.length > 0) {
          const { error } = await supabase.from("ons_flows").insert(processedData)
        ...
        setMessage({
          type: "success",
          text: `${processedData.length} registros nuevos cargados exitosamente`,
        })
        onUploadComplete()
      } catch (error) {
  ...
  return (
    <Card className="w-full">
      <CardHeader>
        ...
        <CardDescription>Sube tu archivo Excel con los datos de ONs para importar automáticamente</CardDescription>
      </CardHeader>
      <CardContent>
        <div
          {...getRootProps()}

...
export function SoberanosDetailsTable({ flows }: SoberanosDetailsTableProps) {
  ...
  function parseLocalISODate(dateString?: string | null) {
    ...
    if (m) {
      const [, y, mo, d] = m
      return new Date(Number(y), Number(mo) - 1, Number(d)) // local time
    }
    const d2 = new Date(s) // timestamps con hora/Z
  ...
  const filteredAndSortedFlows = useMemo(() => {
    ...
    if (sortField) {
      filtered.sort((a, b) => {
        // === ORDEN ESPECIAL: vencimiento con parser local
  ...
  return (
    <div className="space-y-4">
      {/* Header con información específica de Soberanos */}
      <div className="bg-gradient-to-r from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4">
        <div className="flex items-center gap-2 mb-2">
          <DollarSign className="h-5 w-5 text-green-600" />
          <h3 className="font-semibold text-green-800">Soberanos Hard Dollar</h3>
        </div>
        <p className="text-sm text-green-700">
          Bonos soberanos denominados en dólares estadounidenses. Todos los valores están expresados en USD.
        </p>
      </div>
            ...
            {filteredAndSortedFlows.map((flow) => (
              <TableRow key={flow.id}>
                ...
                <TableCell className="text-center bg-rose-50">
                  {(() => {
                    const d = parseLocalISODate(flow.details?.fecha_vencimiento || null)
      ...
      <div className="text-xs text-muted-foreground text-center">
        Mantén Shift + rueda del mouse para desplazarte horizontalmente • Mostrando {filteredAndSortedFlows.length} de {flows.length} registros de Soberanos Hard Dollar
      </div>
    </div>
  )
}
